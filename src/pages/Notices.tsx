...
export function Notices() {
  const { notices, activeGroupId, addNotice, updateNotice, deleteNotice, currentUserRole } = useAppContext();
  ...
  const handleUpdate = (id: string) => {
    ...
    if (!n) return;
    updateNotice({ ...n, title: editTitle, content: editContent });
    setEditingId(null);
  };
      ...
      <div className={`space-y-4 ${canEdit ? 'lg:col-span-8' : 'lg:col-span-12'}`}>
        ...
        {groupNotices.map(notice => {
          if (editingId === notice.id) {
            return (
              <div key={notice.id} className="bento-card border-l-4 border-l-app-primary space-y-4">
                <input type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} className="bento-input w-full" />
                <textarea rows={4} value={editContent} onChange={e => setEditContent(e.target.value)} className="bento-input w-full" />
                <div className="flex justify-end gap-2">
                  <button onClick={() => setEditingId(null)} className="px-3 py-1 bg-slate-200 rounded">Cancel</button>
                  <button onClick={() => handleUpdate(notice.id)} className="px-3 py-1 bg-blue-600 text-white rounded">Save</button>
                </div>
              </div>
            );
          }
          ...
          return (
            <div key={notice.id} className="bento-card border-l-4 border-l-app-primary">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {canEdit && (
                      <div className="flex gap-1">
                        <button onClick={() => { setEditingId(notice.id); setEditTitle(notice.title); setEditContent(notice.content); }} className="p-1 text-slate-400 hover:text-blue-600"><Pencil className="w-3.5 h-3.5"/></button>
                        <button onClick={() => handleDelete(notice.id)} className="p-1 text-slate-400 hover:text-red-600"><Trash2 className="w-3.5 h-3.5"/></button>
                      </div>
                    )}
                  </div>
              ...
              <p className="text-sm text-app-text whitespace-pre-wrap">{notice.content}</p>
            </div>
          );
        })}
      </div>
